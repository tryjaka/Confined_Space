...
func _physics_process(delta):
	camera.position = Vector3(ball.position.x, max(camera_y, ball.position.y + 20), ball.position.z)
...
func _build_maze(cell_size: float, offset: float) -> void:
	...
	for row in maze.number_rows:
		for column in maze.number_columns:
			...
			var floor_ = Floor.instantiate()
			floor_.mesh.size = Vector2(cell_size, cell_size)
			floor_.position = Vector3(left_boundary, 0, bottom_boundary)
			add_child(floor_)

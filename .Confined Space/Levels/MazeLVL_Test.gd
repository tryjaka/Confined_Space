...
var width = 95  # width of map (in tiles)
var height = 95  # height of map (in tiles)
...
func _physics_process(delta):
	if Input.is_action_just_pressed("ui_focus_next"):
		$Camera2D.set_zoom(Vector2(18, 18));
...
func make_maze():
	...
	while unvisited:
		...
		if neighbors.size() > 0:
			...
			#print_debug("visited stack",stack)
			# remove walls from *both* cells
			var dir = next - current
			#print_debug("Dir = Next - Current -> ",next,"-", current," = ", dir,", ",[dir],", ",[-dir])
			var current_walls = Map.get_cellv(current) - cell_walls[dir]
			#print_debug("Current Tile = ",Map.get_cellv(current),"| cr cell dir = ",cell_walls[dir],"| cr_wall ======= ",current_walls)
			var next_walls = Map.get_cellv(next) - cell_walls[-dir]
			#print_debug("Next Tile = ",Map.get_cellv(next),"| nx cell dir = ",cell_walls[-dir],"| nx_wall = ",next_walls)
		...
		time_now = OS.get_ticks_usec()
	
	var time_elapsed = time_now - time_start
	print("Maze start time = ",time_start)
	print("Maze now time = ",time_now)
	print("Maze generation time = ",time_elapsed)
